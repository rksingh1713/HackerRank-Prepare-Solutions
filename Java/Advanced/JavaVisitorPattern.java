...
class ProductOfRedNodesVisitor extends TreeVis {
  ...
  void multiply(Tree tree) {
    if (tree.getColor() == Color.RED) productOfRedNodes =
      (productOfRedNodes * tree.getValue()) % (1000000007);
  }
...
public class JavaVisitorPattern {
  ...
  public static Tree solve() {
    ...
    for (int i = 0; i < n; i++) nodeAtts.put(
      i + 1,
      new Object[] { sc.nextInt(), null }
    );

    for (int i = 0; i < n; i++) nodeAtts.get(i + 1)[1] =
      sc.nextInt() == 0 ? Color.RED : Color.GREEN;

    Map<Integer, ArrayList<Integer>> edges = new HashMap<Integer, ArrayList<Integer>>();

    for (int i = 1; i <= n; i++) edges.put(i, new ArrayList<Integer>());
    ...
    Tree root = new TreeNode(
      (Integer) nodeAtts.get(1)[0],
      (Color) nodeAtts.get(1)[1],
      0
    );
    tree.put(1, root);
  ...
  private static void DFSUtil(
    ...
    if (v != 1) {
      ...
      tree.put(v, treeNode);
    } else treeNode = (TreeNode) tree.get(1);
